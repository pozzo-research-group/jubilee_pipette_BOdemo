...
class AxSolver():
    ...
    def get_ax_object(n_params, n_random_trials, n_bo_trials):
        ...
        experiment = ax_client.create_experiment(
                name="color_matching",
                parameters=[{"name":f"x{i+1}", "type":"range", "bounds":[0.0, 1.0], "value_type":"float"} for i in range(n_params)], 
                objectives={"euclidean": ObjectiveProperties(minimize=True)},
                parameter_constraints=[' + '.join([f'x{i+1}' for i in range(n_params)]) + " <= 1.0"]  # Optional.
        )
    ...
    def ask(self):
        ...
        return list(query_point.values())
    
    def update(self, x_data, y_data):

        y_dict = {'euclidean':(y_data[0], 0)}
